...
class Robot():
    ...
    def get_connection_status(self):
        """
            ...
            Returns:
                ...
                Different connection IDs indicate temporary disconnections in-between.
        """
    ...
    def start_sensors(self):
        """
            ...
            Returns:
                us_handle: List that contains each ultrasonic sensor handle ID.
...
def reset(connection_address="127.0.0.1", connection_port=19997):
    client_id = vrep.simxStart(connection_address, connection_port, True, True, 2000, 5)
    try:
        res = vrep.simxStopSimulation(client_id, vrep.simx_opmode_oneshot_wait)
        if res == vrep.simx_return_ok:
...
def release(robot):
    robot.command = ""
    robot.stop()
    vrep.simxFinish(robot.clientID)
